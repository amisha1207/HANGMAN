...
while run:
    ...
    file=open('animals_easy.txt','r')
    l=file.readlines()
    selected_word=(l[index].strip('/n')).upper()
    #print(selected_word)

    #hints 
    hints=open('animalhints_easy.txt','r')
    p=hints.readlines()
    ...
    ex = Button(hang,bd = 0,command = lambda:close(),bg="#E7FFFF",activebackground = "#E7FFFF",font = 10,image = e1)
    ex.place(x=770,y=10)
    s2 = 'SCORE:'+str(score)
